// app/privacy/page.tsx — Πολιτική Απορρήτου (placeholder)
import type { Metadata } from "next";
import Link from "next/link";

export const metadata: Metadata = {
  title: "Πολιτική Απορρήτου | Trustia.gr",
};

export default function PrivacyPage() {
  return (
    <main style={{ maxWidth: "760px", margin: "0 auto", padding: "4rem 1.5rem" }}>
      <p style={{ fontSize: "0.8rem", color: "var(--color-text-muted)", marginBottom: "0.5rem", textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 600 }}>
        Νομικά
      </p>
      <h1 style={{ fontSize: "clamp(1.75rem, 4vw, 2.5rem)", fontWeight: 800, color: "var(--color-text)", margin: "0 0 1rem" }}>
        Πολιτική Απορρήτου
      </h1>
      <p style={{ fontSize: "0.875rem", color: "var(--color-text-muted)", marginBottom: "2.5rem" }}>
        Τελευταία ενημέρωση: Μάιος 2026
      </p>

      <div
        style={{
          backgroundColor: "rgba(212,160,57,0.08)",
          border:          "1.5px solid rgba(212,160,57,0.3)",
          borderRadius:    "12px",
          padding:         "1.25rem 1.5rem",
          marginBottom:    "2.5rem",
          display:         "flex",
          gap:             "0.75rem",
          alignItems:      "flex-start",
        }}
      >
        <span style={{ fontSize: "1.25rem", flexShrink: 0 }}>🚧</span>
        <div>
          <p style={{ fontWeight: 700, color: "#92400E", margin: "0 0 0.25rem" }}>
            Σε εξέλιξη
          </p>
          <p style={{ fontSize: "0.875rem", color: "#92400E", margin: 0, lineHeight: 1.6 }}>
            Η πλήρης Πολιτική Απορρήτου (GDPR-συμβατή) βρίσκεται σε νομική επεξεργασία.
            Για ερωτήσεις:{" "}
            <a href="mailto:info@trustia.gr" style={{ color: "#92400E", fontWeight: 700 }}>
              info@trustia.gr
            </a>
          </p>
        </div>
      </div>

      <section style={{ lineHeight: 1.8, color: "var(--color-text)", fontSize: "0.9375rem" }}>
        <h2 style={{ fontWeight: 700, fontSize: "1.125rem", margin: "0 0 0.75rem" }}>1. Υπεύθυνος Επεξεργασίας</h2>
        <p style={{ marginBottom: "1.5rem" }}>
          Trustia.gr · Θεσσαλονίκη, Ελλάδα ·{" "}
          <a href="mailto:info@trustia.gr" style={{ color: "var(--color-primary)", fontWeight: 600 }}>info@trustia.gr</a>
        </p>

        <h2 style={{ fontWeight: 700, fontSize: "1.125rem", margin: "0 0 0.75rem" }}>2. Δεδομένα που Συλλέγουμε</h2>
        <ul style={{ marginBottom: "1.5rem", paddingLeft: "1.25rem" }}>
          <li>Ονοματεπώνυμο, email, τηλέφωνο κατά την εγγραφή</li>
          <li>Τοποθεσία (πόλη / περιοχή) για αποτελέσματα αναζήτησης</li>
          <li>Δεδομένα χρήσης (σελίδες που επισκεφθήκατε, αναζητήσεις)</li>
        </ul>

        <h2 style={{ fontWeight: 700, fontSize: "1.125rem", margin: "0 0 0.75rem" }}>3. Χρήση Δεδομένων</h2>
        <p style={{ marginBottom: "1.5rem" }}>
          Τα δεδομένα χρησιμοποιούνται αποκλειστικά για τη λειτουργία της πλατφόρμας,
          τη σύνδεση επαγγελματιών–πελατών και (με συγκατάθεση) για ενημερωτικά email.
        </p>

        <h2 style={{ fontWeight: 700, fontSize: "1.125rem", margin: "0 0 0.75rem" }}>4. Δικαιώματά σας (GDPR)</h2>
        <p style={{ marginBottom: "1.5rem" }}>
          Δικαίωμα πρόσβασης, διόρθωσης, διαγραφής, φορητότητας και εναντίωσης.
          Επικοινωνήστε στο{" "}
          <a href="mailto:info@trustia.gr" style={{ color: "var(--color-primary)", fontWeight: 600 }}>info@trustia.gr</a>.
        </p>
      </section>

      <div style={{ marginTop: "2.5rem", paddingTop: "1.5rem", borderTop: "1px solid var(--color-border)" }}>
        <Link href="/" style={{ color: "var(--color-primary)", fontWeight: 600, fontSize: "0.9rem", textDecoration: "none" }}>
          ← Αρχική
        </Link>
      </div>
    </main>
  );
}
